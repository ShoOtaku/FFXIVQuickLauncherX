...
namespace XIVLauncher.Common.Encryption.BlockCipher
{
    internal unsafe struct BlowfishState
    {
        ...
        // http://www.herongyang.com/Blowfish/Algorithm-Blowfish-Cipher-Encryption-Algorithm.html
        // https://en.wikipedia.org/wiki/Blowfish_(cipher)

        private const int Rounds = 16;
        public const int PSize = 18;
        public const int SSize = 256;

        private static readonly uint[] PInit = new uint[]
        {
        ...
        private static readonly uint[] S0Init = new uint[]
        {
        ...
        private static readonly uint[] S1Init = new uint[]
        {
        ...
        private static readonly uint[] S2Init = new uint[]
        {
        ...
        private static readonly uint[] S3Init = new uint[]
        {
        ...
        public BlowfishState(ReadOnlySpan<byte> key)
        {
            ...
            fixed (uint* pDst = m_p)
            {
                Buffer.MemoryCopy(pSrc, pDst, PSize * 4, PSize * 4);
            }

            fixed (uint* pSrc = S0Init)
            fixed (uint* pDst = m_s0)
            {
                Buffer.MemoryCopy(pSrc, pDst, SSize * 4, SSize * 4);
            }

            fixed (uint* pSrc = S1Init)
            fixed (uint* pDst = m_s1)
            {
                Buffer.MemoryCopy(pSrc, pDst, SSize * 4, SSize * 4);
            }

            fixed (uint* pSrc = S2Init)
            fixed (uint* pDst = m_s2)
            {
                Buffer.MemoryCopy(pSrc, pDst, SSize * 4, SSize * 4);
            }

            fixed (uint* pSrc = S3Init)
            fixed (uint* pDst = m_s3)
            {
                Buffer.MemoryCopy(pSrc, pDst, SSize * 4, SSize * 4);
            }
        ...
        /// <summary>
        /// Initializes P-array and S-boxes with given key. P-array and S-boxes must be initialized to initial values beforehand.
        /// </summary>
